 ...
 * @see randomUniform(std::mt19937& rng)
 */
double randomUniform();

/**
 * @brief Generates a standard normal random double (mean = 0, variance = 1) by inverse transform
 * sampling.
 *
 * Uses randomUniform to produce a uniform sample and the inverse of the
 * CDF of the normal distribution to obtain the normal sample.
 *
 * @param rng A reference to a random number engine.
...
/**
 * @brief Thread-safe generation of a standard normal random double (mean = 0, variance = 1).
 *
 * Accesses the internal global Mersenne twister engine and passes it into randomNormal.
 *
 * @return A pseudo-random double.
 *
 * @note This function is thread-safe since it wraps the call in a std::lock_guard.
 * @see randomNormal(std::mt19937& rng)
 */
double randomNormal();
